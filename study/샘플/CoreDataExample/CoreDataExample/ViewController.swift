...
extension ViewController {
  ...
  func fetchData() {
    ...
    do {
      let result = try context.fetch(request) //질문 보내고 결과값 받아온다
      _ = result.map { print("name : ", $0.name ?? "None", "age : \($0.age)") }
        
//        for person in result{ 맵에서 이런 일은 했음
//            print("name : ", $0.name ?? "None", "age : \($0.age)")
//        }
  ...
  func changeName(from: String, to: String) {
    let request: NSFetchRequest<Person> = Person.fetchRequest()
    request.predicate = NSPredicate(format: "name = %@", from)//정규표현식 알아놓는게 좋다 NSPredicate 정규표현식
